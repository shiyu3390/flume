...
public class RegexExtractorInterceptor implements Interceptor {
  ...
  private RegexExtractorInterceptor(Pattern regex,
      List<NameAndSerializer> serializers,String regexString) {
    this.regex = regex;
  ...
  public Event intercept(Event event) {
    ...
    if (matcher.find()) {
      for (int group = 0, count = matcher.groupCount(); group < count; group++) {
        ...
        if ("logContent".equals(serializer.headerName)) {
          event.setBody(serializer.serializer.serialize(matcher.group(groupIndex)).getBytes());
        }else {
    ...
    for (int i = 0;i < serializers.size();i++){
      NameAndSerializer serializer = serializers.get(i);
      if (headers.get(serializer.headerName) == null && "logDate".equals(serializer.headerName)) {
        headers.put(serializer.headerName, sdf.format(new Date()));
      }
      if (headers.get(serializer.headerName) == null && "logLevel".equals(serializer.headerName)) {
        headers.put(serializer.headerName, "");
      }
    }
    headers.put("regex",regexString);
    return event;
  }
  ...
  public static class Builder implements Interceptor.Builder {
    ...
    public void configure(Context context) {
      regexStr = context.getString(REGEX);
      Preconditions.checkArgument(!StringUtils.isEmpty(regexStr),
    ...
    public Interceptor build() {
      ...
      return new RegexExtractorInterceptor(regex, serializerList,regexStr);
    }
  }
  ...
  public static void main(String[] args) {
    byte[] bytes = "stststs".getBytes();
    byte[] encode = Base64.getEncoder().encode(bytes);
    System.out.println(encode);
    String encodeToString = Base64.getEncoder().encodeToString(bytes);
    System.out.println(encodeToString);
    System.out.println(new String(Base64.getDecoder().decode(encodeToString)));
    String regex = "[\\S\\s]*";
    String msg = "{\"Payload\":\"Sep 20 11:51:45 haproxy[12]: Server rabbitmq_management/sr-kvm-control-46 is UP, reason: Layer4 check passed, check duration: 0ms. 3 active and 0 backup servers online. 0 sessions requeued, 0 total in queue.\"}";
