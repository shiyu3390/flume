 ...
 * under the License.
 */
package org.apache.flume.sinorail.channel.kafka;
...
public class KafkaChannel extends BasicChannelSemantics {
    ...
    private boolean migrateZookeeperOffsets = KafkaChannelConfiguration.DEFAULT_MIGRATE_ZOOKEEPER_OFFSETS;
    ...
    private long pollTimeout = KafkaChannelConfiguration.DEFAULT_POLL_TIMEOUT;
  ...
  */
    ...
    public void configure(Context ctx) {
        ...
        topicStr = ctx.getString(KafkaChannelConfiguration.TOPIC_CONFIG);
        if (topicStr == null || topicStr.isEmpty()) {
            topicStr = KafkaChannelConfiguration.DEFAULT_TOPIC;
            logger.info("Topic was not specified. Using {} as the topic.", topicStr);
        }
        topic.set(topicStr);
        groupId = ctx.getString(KafkaChannelConfiguration.KAFKA_CONSUMER_PREFIX + ConsumerConfig.GROUP_ID_CONFIG);
        if (groupId == null || groupId.isEmpty()) {
            groupId = KafkaChannelConfiguration.DEFAULT_GROUP_ID;
        ...
        parseAsFlumeEvent = ctx.getBoolean(KafkaChannelConfiguration.PARSE_AS_FLUME_EVENT, KafkaChannelConfiguration.DEFAULT_PARSE_AS_FLUME_EVENT);
        pollTimeout = ctx.getLong(KafkaChannelConfiguration.POLL_TIMEOUT, KafkaChannelConfiguration.DEFAULT_POLL_TIMEOUT);

        staticPartitionId = ctx.getInteger(KafkaChannelConfiguration.STATIC_PARTITION_CONF);
        partitionHeader = ctx.getString(KafkaChannelConfiguration.PARTITION_HEADER_NAME);

        migrateZookeeperOffsets = ctx.getBoolean(KafkaChannelConfiguration.MIGRATE_ZOOKEEPER_OFFSETS,
                KafkaChannelConfiguration.DEFAULT_MIGRATE_ZOOKEEPER_OFFSETS);
        zookeeperConnect = ctx.getString(KafkaChannelConfiguration.ZOOKEEPER_CONNECT_FLUME_KEY);
    ...
    private void translateOldProps(Context ctx) {
        ...
        if (!(ctx.containsKey(KafkaChannelConfiguration.TOPIC_CONFIG))) {
            ctx.put(KafkaChannelConfiguration.TOPIC_CONFIG, ctx.getString("topic"));
        ...
        if (!(ctx.containsKey(KafkaChannelConfiguration.BOOTSTRAP_SERVERS_CONFIG))) {
            String brokerList = ctx.getString(KafkaChannelConfiguration.BROKER_LIST_FLUME_KEY);
            if (brokerList == null || brokerList.isEmpty()) {
                throw new ConfigurationException("Bootstrap Servers must be specified");
            } else {
                ctx.put(KafkaChannelConfiguration.BOOTSTRAP_SERVERS_CONFIG, brokerList);
                logger.warn("{} is deprecated. Please use the parameter {}",
                        KafkaChannelConfiguration.BROKER_LIST_FLUME_KEY, KafkaChannelConfiguration.BOOTSTRAP_SERVERS_CONFIG);
            }
        }

        //GroupId
        // If there is an old Group Id set, then use that if no groupId is set.
        if (!(ctx.containsKey(KafkaChannelConfiguration.KAFKA_CONSUMER_PREFIX + ConsumerConfig.GROUP_ID_CONFIG))) {
            String oldGroupId = ctx.getString(KafkaChannelConfiguration.GROUP_ID_FLUME);
            if (oldGroupId != null && !oldGroupId.isEmpty()) {
                ctx.put(KafkaChannelConfiguration.KAFKA_CONSUMER_PREFIX + ConsumerConfig.GROUP_ID_CONFIG, oldGroupId);
                logger.warn("{} is deprecated. Please use the parameter {}",
                        KafkaChannelConfiguration.GROUP_ID_FLUME, KafkaChannelConfiguration.KAFKA_CONSUMER_PREFIX + ConsumerConfig.GROUP_ID_CONFIG);
            }
        }

        if (!(ctx.containsKey((KafkaChannelConfiguration.KAFKA_CONSUMER_PREFIX + ConsumerConfig.AUTO_OFFSET_RESET_CONFIG)))) {
            Boolean oldReadSmallest = ctx.getBoolean(KafkaChannelConfiguration.READ_SMALLEST_OFFSET);
            String auto;
            if (oldReadSmallest != null) {
                ...
                ctx.put(KafkaChannelConfiguration.KAFKA_CONSUMER_PREFIX + ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, auto);
                logger.warn("{} is deprecated. Please use the parameter {}",
                        KafkaChannelConfiguration.READ_SMALLEST_OFFSET,
                        KafkaChannelConfiguration.KAFKA_CONSUMER_PREFIX + ConsumerConfig.AUTO_OFFSET_RESET_CONFIG);
            }
    ...
    private void setProducerProps(Context ctx, String bootStrapServers) {
        producerProps.put(ProducerConfig.ACKS_CONFIG, KafkaChannelConfiguration.DEFAULT_ACKS);
        producerProps.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, KafkaChannelConfiguration.DEFAULT_KEY_SERIALIZER);
        producerProps.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, KafkaChannelConfiguration.DEFAULT_VALUE_SERIAIZER);
        //Defaults overridden based on config
        producerProps.putAll(ctx.getSubProperties(KafkaChannelConfiguration.KAFKA_PRODUCER_PREFIX));
    ...
    private void setConsumerProps(Context ctx, String bootStrapServers) {
        consumerProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, KafkaChannelConfiguration.DEFAULT_KEY_DESERIALIZER);
        consumerProps.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, KafkaChannelConfiguration.DEFAULT_VALUE_DESERIAIZER);
        consumerProps.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, KafkaChannelConfiguration.DEFAULT_AUTO_OFFSET_RESET);
        //Defaults overridden based on config
        consumerProps.putAll(ctx.getSubProperties(KafkaChannelConfiguration.KAFKA_CONSUMER_PREFIX));
...
//  private static Long currTime = System.currentTimeMillis();
    ...
    private class KafkaTransaction extends BasicTransactionSemantics {
        ...
        protected void doPut(Event event) throws InterruptedException {
            ...
            //添加时间戳
            String logDate = event.getHeaders().get("logDate");
            long timestamp = System.currentTimeMillis();
            try {
                timestamp = sdf.parse(logDate).getTime();
            ...
            try {
                ...
                if (partitionId != null) {
                    producerRecords.get().add(
                            new ProducerRecord<String, byte[]>(topic.get(), partitionId, timestamp, key,
                                    serializeValue(event, parseAsFlumeEvent)));
                } else {
                    producerRecords.get().add(
                            new ProducerRecord<String, byte[]>(topic.get(), null,timestamp, key,
                                    serializeValue(event, parseAsFlumeEvent)));
                }
            ...
            if (!consumerAndRecords.get().failedEvents.isEmpty()) {
                ...
                try {
                    ...
                    if (consumerAndRecords.get().recordIterator.hasNext()) {
                        ...
                        if (record.key() != null) {
                            e.getHeaders().put(KafkaChannelConfiguration.KEY_HEADER, record.key());
                        }
        ...
        private byte[] serializeValue(Event event, boolean parseAsFlumeEvent) throws IOException {
            ...
            String logDate = headers.get("logDate");
            String logLevel = headers.get("logLevel");
...
//                file = "";
//            }
            String pkg = event.getHeaders().get("package");
            if (pkg == null) {
                pkg = "";
            }

            String message = logDate + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    logLevel + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    pkg + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    pid + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    new String(event.getBody());

            String prefix = headers.get("isCloud") + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    headers.get("hostIp") + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    headers.get("hostname") + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    headers.get("cluster") + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    headers.get("module") + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    headers.get("role") + KafkaChannelConfiguration.KEY_SPLIT_STR +
                    headers.get("logFileName") + KafkaChannelConfiguration.KEY_SPLIT_STR;

            byte[] content = (prefix + KafkaChannelConfiguration.KEY_VALUE_SPLIT_STR + message).getBytes();
